...
namespace LinqUebung1.App
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int result5 = db.Schuelers.SelectMany(s => s.Pruefungen).Where(s => s.Pruefer == "FAV").Select(x=>x.Note).Max();
            ...
            double result6 = db.Schuelers.Where(s => s.Geschlecht == "w").SelectMany(x => x.Pruefungen.Where(s => s.Fach == "POS")).Average(x=>x.Note);
